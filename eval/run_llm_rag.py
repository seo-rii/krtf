...
from .metrics import (manifest_provenance, run_manifest,
                      wilson_interval)
...
def sample_silver_sentences(corpus, glossary, n: int) -> list[dict]:
    ...
    rows = []
    for row in corpus:
        occs = silver_occurrences(row["text"], silver_aliases)
        if occs:
...
def main():
    ...
    if args.track in ("easy", "both"):
        results["ktrf"]["manifest_easy"] = run_manifest(ROOT)
        ...
        for spec in specs:
            ...
            try:
                ...
                results.setdefault(model, {}).update({
                    ...
                    "manifest_easy": run_manifest(ROOT),
                })
    ...
    if args.track in ("hard", "both"):
        ...
        results["ktrf"]["manifest_hard"] = run_manifest(ROOT)
        ...
        for spec in specs:
            ...
            try:
                results.setdefault(model, {}).update({
                    ...
                    "hard_queries": n,
                    "manifest_hard": run_manifest(ROOT),
                })
...
def write_markdown(payload: dict, out_path: Path) -> None:
    ...
    # `--track hard` merges into a previous easy-track payload, so a row
    # here can be days and commits older than the row above it. Stamping
    # each (model, track) unit is the only way the table can say so; the
    # report carried no provenance at all before.
    manifests = {}
    for name, block in r.items():
        if not isinstance(block, dict):
            continue
        for track in ("easy", "hard"):
            m = block.get(f"manifest_{track}")
            if m:
                manifests[f"{name} ({track})"] = m
    block = manifest_provenance(manifests)
    lines += block or [
        "- 이 페이로드는 per-run manifest 도입 전에 생성되었다 —"
        " 행별 측정 시점을 알 수 없다."]
    lines += [
        "",
        # Not the shared provenance footer: that one stamps the process
        # doing the rendering, and this table's rows were measured by other
        # processes on other days. Render time and measurement time are
        # different facts; conflating them is what the per-run manifests
        # above exist to stop.
        f"*이 마크다운은 {time.strftime('%Y-%m-%d')}에 렌더링되었다 —"
        + (" 측정 시점은 위 행별 manifest가 말한다.*" if block
           else " 이 페이로드의 측정 시점은 기록되어 있지 않다.*"),
        "",
