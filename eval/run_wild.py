"""Wild-corpus benchmark on real Korean text (HuggingFace, `eval/wild_data`).

Usage: python -m eval.run_wild [--corpus wild|holdout|holdout2|web]
                               [--render-only eval/out/wild*.json]

Three suites over a named corpus. `wild` is what the published report
measures — 114,605 sentences across eleven sources, not the ~5k KLUE
headlines this docstring described for as long as it was the only corpus.
The other names are held out or a different genre; each writes its own
report, because one path for every corpus lets the last run stand in for a
distribution it never read:
...
1. **Silver recall** (realorg glossary): occurrences of unambiguous real-org
   ...
   in running text these are near-certain mentions. Measures E2E detection,
...
Writes eval/out/wild[_<corpus>].json and reports/WILD_CORPUS[_<CORPUS>].md.
"""
...
def write_markdown(payload: dict, out_path: Path) -> None:
    ...
    name = payload.get("corpus", "wild")
    flag = "" if name == "wild" else f" --corpus={name}"
    lines = [
        f"# KTRF Wild-Corpus 벤치마크 — `{name}` (실제 한국어 텍스트)",
        "",
        f"코퍼스: HuggingFace 공개 한국어 텍스트"
        f" {payload['corpus_sentences']}문장. 도메인 구성은 아래 소스"
        " 목록이 전부다 — 이 줄에 도메인을 적어 두면 다른 코퍼스를 잰"
        " 리포트에도 같은 글이 찍힌다."
        " 합성 데이터와 달리 구현 카탈로그와 독립적인 실 분포다."
        f" 재현: `python -m eval.run_wild{flag}` (최초 실행 시 다운로드).",
        "",
        "소스 구성: " + ", ".join(
            f"`{k.split('/')[-1]}` {v}"
        ...
        "",
        "무모호 표면형(정부기관 전체명·3자 이상 약칭)의 실문 출현은 사실상"
    ...
    lines += [
        ...
        "- 이 코퍼스가 어떤 문체인지는 위 소스 목록이 말한다. 어느"
        " 구성이든 사내 문서 분포는 아니며, 그쪽은 tenant golden"
        " set(§48.6)으로만 검증할 수 있다.",
        "- tail 커버리지의 미포함 항목은 §16 카탈로그 확장의 실측 우선순위"
        " 신호로 사용한다 (OQ-001).",
        "",
        provenance_line(ROOT, manifest=payload.get("manifest")),
        "",
        f"*generated by `python -m eval.run_wild{flag}`*",
    ]
    out_path.write_text("\n".join(lines), encoding="utf-8")


def main():
    # no `description=__doc__`: the module docstring is Korean, and
    # argparse writes help to a console that on Windows is cp949, so
    ...
    out = ROOT / "eval" / "out"
    # a named corpus gets its own files: one path for every corpus would let
    # the last run silently stand in for a distribution it never read
    suffix = "" if args.corpus == "wild" else f"_{args.corpus}"
    json_path = out / f"wild{suffix}.json"
    md_path = ROOT / "reports" / f"WILD_CORPUS{suffix.upper()}.md"

    if args.render_only:
        payload = json.loads(
            Path(args.render_only).read_text(encoding="utf-8"))
        if not payload.get("manifest"):
            # Without it the footer would stamp today's HEAD and today's
            # date on numbers of unknown age — the one claim this report
            # must never make.
            raise SystemExit(
                f"{args.render_only} has no run manifest, so re-rendering "
                f"could only guess when and at what commit it was "
                f"measured; re-run the benchmark instead.")
        try:
            write_markdown(payload, md_path)
        except KeyError as missing:
            # A payload written by an older renderer is missing fields this
            # one reads. Half a report is worse than none: it would carry
            # today's prose over yesterday's shape and look complete.
            raise SystemExit(
                f"{args.render_only} predates this report format "
                f"(no {missing}); re-run the benchmark instead of "
                f"re-rendering.") from None
