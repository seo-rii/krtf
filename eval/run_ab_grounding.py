...
def build_cases(corpus, glossary,
    ...
    known: list[dict] = []
    for row in corpus:
        occs = silver_occurrences(row["text"], abbrevs)
        for s, e, a in occs:
            eid = alias_to[a][0]
            known.append({
...
def _private_slice(corpus, glossary, rng, n: int):
    ...
    hosts = []
    for row in corpus:
        occs = silver_occurrences(row["text"], donors)
        if occs:
            s, e, a = occs[0]
...
def write_markdown(payload: dict, out_path: Path) -> None:
    ...
    # Each model is a separate `--model` invocation merged into the same
    # payload, so there is no single manifest for this table. Printing the
    # first run's was a claim about rows that had never been compared: two
    # models measured a week and four commits apart would have rendered
    # identically to two measured back to back.
    lines += manifest_provenance(
        # `model` is the row key, so it differs by construction; leaving it
        # in would fire the disagreement warning on every report ever
        # generated and teach the reader to skip it.
        {model: {k: v for k, v in run.get("manifest", {}).items()
                 if k != "model"}
         for model, run in runs.items()})
    lines += [
        "",
