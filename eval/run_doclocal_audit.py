...
def write_markdown(t: dict, control: dict | None, out_path: Path) -> None:
    ...
    lines += [
        ...
        "### 이 서술이 어느 코퍼스에서 나왔는지",
        "",
        "아래 세 문단은 **`wild` 코퍼스를 읽고 찾은 결함의 기록**이며, 이"
        " 리포트가 재는 코퍼스에 대한 주장이 아니다. 같은 글이 모든 코퍼스"
        " 리포트에 그대로 찍히고 있었으므로 — 다른 코퍼스의 수치 아래에"
        " `wild`의 이야기가 실렸다 — 출처를 밝혀 분리한다. 이 리포트가"
        " 재는 코퍼스에 대한 사실은 위 표와 아래 목록뿐이다.",
        "",
        "**하나.** 전 코퍼스에서 나온 별칭을 하나씩 읽어 보니 **`Y(X)`"
        " 갈래가 세 번 발동해 세 번 틀렸다** — `노선영(강원도청)`은"
        " 선수(소속팀), `현대캐피탈(현대자동차그룹)`은 자회사(모기업)다."
        " 그 갈래의 유일한 증거가 '괄호 안이 더 길다'였기 때문이고, 더"
        " 길다는 것은 증거가 아니다. 이제 한글 약칭은 부분수열 정렬을"
        " 보여야 한다.",
        "",
        "**둘.** 긴 이름을 괄호 **앞 전체**와 견주고 있었다. 실제 문장에서"
        " 괄호 앞은 이름만이 아니므로 — `이종호 과학기술정보통신부 이하"
        " 과기정통부 장관은` — 무관한 앞말의 길이가 정의 인정 여부를"
        " 갈랐다. 이름은 괄호에 붙어 있는 쪽이므로 거기서부터 맞춘다.",
        "",
        "**셋.** 괄호만으로는 정의가 아니다. `서울특별시(사실상),"
        " 세종특별자치시(행정)`은 어느 수도가 어떤 의미인지를 적은 줄이지"
        " 정의가 아니다. `이하`가 명시하거나, 약칭 정렬이 서거나, 문서가"
        " 그 이름을 실제로 다시 쓰거나 — 셋 중 하나가 필요하다.",
        "",
        "**넷.** 한 별칭에 긴 이름이 둘이면 정의가 아니다."
        " `삼성전자(Aa3)와 SK텔레콤(A3), KT(A3)`에서 A3는 신용등급이고,"
        " 목록 안의 코드는 당연히 반복되므로 '문서가 다시 쓴다'는 조건을"
        " **다음 항목이** 채워 준다. 같은 긴 이름의 반복은 정상이다 —"
        " 기사 하나가 자기 약칭을 네 번 다시 쓰는 것은 정의 하나다.",
        "",
        "**둘·셋은 `wild`를 읽고 골랐으므로 그 코퍼스에 과적합됐을 수"
        " 있다.** 규칙을 한 번도 비춰 보지 않은 코퍼스에서 재보면"
        " `holdout` 3건(`과기정통부`·`코레일`·`방통위`, 이전 0건),"
        " `holdout3` 13건(`기재부`·`중기부`·`복지부`·`전경련`·`농식품부`"
        "·`LGD`·`코레일`·`과기정통부` 등), `web` 1건(`공수처`)이고"
        " 오탐은 없다 — 127,735문장에서 잃고 있던 정의 열일곱을 되찾았고"
        " 잘못 만든 것은 없다. 넷은 그 `web`이 찾아 준 결함이다.",
        "",
