...
| core span 오분해율 | `\|mention` | 0.0052 | 0.0052 |
| 잘못된 entity 확정 | `\|commit` | 0 | 0 |
| **불변조건 ② 위반** | `\|commit` | **0** | **0** |
| **넓은 표면형 `UNKNOWN` 판정** | — | **639 / 1,070** | **352 / 1,070** |

읽는 법: **재현율도 확정도 span도 움직이지 않았고, 이름을 못 붙이던 표면형만
...
0이다 — 근접 표면형이 후보로는 172번 올라왔는데(시험이 살아 있다는 증거)
확정으로는 한 번도 넘어가지 않았다.
...
| 지표 | 조건 | M2 | M3 |
|---|---|---:|---:|
| 응답에 실린 mention | — | 160 | **147** |
| mention precision | `\|mention` | 0.3688 | 0.4014 |
| 단어를 자른 span | `\|mention` | 0.0875 (14) | **0.0612 (9)** |
| gold entity가 후보에 | `\|candidate` | 1.0 | 1.0 |
| commit precision | `\|commit` | 0.931 | 0.9333 |
...
| mention | 3,867 | **3,611** |
| RESOLVED 확정 | 771 | **775** |
| core보다 넓은 표면형 | 479 | 499 |
| 그중 `UNKNOWN` | 102 | **79** |
| commit 보류(typed) | 742 | 821 |
...
**읽는 법.** M3는 실텍스트에서 **mention을 256건 줄이고 확정을 4건 늘렸다**.
줄어든 것은 `서` 제약과 짧은 라틴 토큰 제한이 걷어낸 잘못된 분해이고 —
gold의 "단어를 자른 span" 14→9가 같은 것을 다른 각도에서 센다 — 늘어난
확정은 조사·종결어를 더 설명하게 된 직접 효과다.
...
**감사가 잡아낸 결함.** abbreviation 정렬을 문자 체계 무관으로 바꾸자 두 글자
