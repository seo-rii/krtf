...
# §16.2 이름의 끝음절이 될 수 없는 조사 — 점수가 같은 두 읽기를 가르는 자리.
#
# 위 주석은 한 음절 조사가 명사의 **첫** 음절과 겹친다고 말하는데(`도`시,
# `과`학), **끝** 음절과도 똑같이 겹친다: 카카오게`임`, 코레일공항철`도`,
# 카카오브레`인`, 서울시메트`로`, 유`엔`. 잔여부가 카탈로그로 설명되지 않을
# 때는 어느 쪽인지 가릴 근거가 없다.
#
# 틀리는 값이 대칭이 아니다. 조사를 이름 안에 남기면 진짜 이름의 superstring을
# 보고하지만(`카카오톡에`), 이름 음절을 조사로 떼면 아무것도 철자하지 않는
# span을 보고하고(`카카오게`) 임은 mention 밖으로 나간다. 그래서 허용 목록이고,
# 여기 없는 조사는 잔여부에 붙은 채로 남는다.
#
# `과`가 이 목록에 있는 것은 SUFFIX_CLASSES와 겹치는 **유일한** 항목이기
# 때문이다. `과`(부서)가 정당한 자리는 `총무과`처럼 카탈로그에 없는 이름 뒤인데,
# 그것은 SUFFIX_WITH_MODIFIER(0.75)로 읽혀 애초에 동점이 아니다. 동점이 되는
# 것은 `서울시장과`·`공정거래법과`처럼 **다른 종결어 바로 뒤**일 때뿐이고,
# 거기서 부서로 읽히는 `과`는 없다 — 접속의 과다.
SPLITTABLE_PARTICLES: frozenset[str] = frozenset({
    # 격조사
    "은", "는", "이", "가", "을", "를", "과", "와", "의",
    # 부사격
    "에", "에서", "에게", "에게서", "한테", "한테서",
    "께", "께서", "에선", "껜",
    "으로", "으로서", "으로써", "로서", "로써",
    "이랑", "부터", "까지", "처럼", "만큼", "보다", "마다",
    # 어절을 닫는 축약형(위 TOKEN_FINAL 제약을 이미 받는다)
    "서",
})
# 일부러 빼둔 것과 그 반례:
#   로(서울시메트`로`, 종`로`)  도(코레일공항철`도`, MBC보`도`)
#   임·인·이다 등 계사·종결(카카오게`임`, 카카오브레`인`)
#   엔(행자부유`엔`)  만·나·든·야·라도  대로(세종`대로`)  랑
