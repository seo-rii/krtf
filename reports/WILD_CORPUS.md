...
>
> **동점을 조사 쪽으로 (`AP통신이`)** — 20,000문장, 두 체크아웃에서
> `run_wild_regression --single`: silver mention 1,176 · 탐지 1.0 ·
> gold-in-set 1.0 · **RESOLVED 1,138 · precision 1.0** · 커버리지 0.9677 ·
> ledger 1,617/479 · tail coverage 0.9273 · **fake FP 0 · 후보 밀도 5.459** —
> **모든 지표가 동일하다.** `_RESIDUAL_BASE`×문법성의 곱 8개가 전부 달라
> 동점은 곧 같은 판정이고, 동점 순서는 **보고하는 이름의 끝**만 움직인다.
> 효과는 감사(이름 안의 조사 25→12)와 스위트(`derivative_particle`의
> `name_span_exact` 0.4673→0.9393)에서 본다.
>
> **붙어 있는 라틴 run을 잔여부로 (`한전KDN`)** — 20,000문장:
>
> | 지표 | 이전 | 이후 |
> |---|---:|---:|
> | RESOLVED commits | 1,141 | **1,138** |
> | **commit precision** | **1.0** | **1.0** |
> | silver 커버리지 | 0.9702 | 0.9677 |
> | tail coverage | 0.9273 | 0.9273 |
> | **fake-glossary FP / 후보 밀도** | **0 / 5.459** | **0 / 5.459** |
>
> 잃은 3건은 전부 `네이버TV`류 서브브랜드이고, 이전에는 그 자리에서 **경고
> 없이 부분 이름을 확정**했다. 구두점을 생략한 병렬 기관명(`산업부KOTRA`)은
> 잔여부가 아니라 다음 mention으로 읽어 두 확정을 모두 살린다.
>
> **혼합 문자 토큰 (`SK하닉`)** — 20,000문장: silver mention·탐지·확정·
> precision·tail coverage·fake-glossary FP·후보 밀도가 **전부 동일**하다.
> 실문장에서 문자 체계를 섞은 run 자체가 드물어 비용이 0이고, 그것이 이
> 변경이 오랫동안 눈에 띄지 않았던 이유이기도 하다. 효과는 합성 스위트의
> `mixed_abbrev` formation에서만 보인다(0.0 → 1.0, n=6).
>
