...
재현: `python -m eval.run_variant_recall` · seed 20260901 · 170 families × 15 formations = **3053 cases** · host 문장 6000개.
...
| 지표 | 조건 | 값 |
|---|---|---:|
| **variant-family macro recall** | `\|candidate` | 0.9392 → **0.9392** (0.0) |
| ├ Level A formation | `\|candidate` | 0.9868 → **0.9868** (0.0) |
| └ **Level B formation** | `\|candidate` | 0.7532 → **0.7532** (0.0) |
| commit macro (§2 SAME 형성) | `\|commit` | 0.9865 → **0.9865** (0.0) |
| core span 오분해율 | `\|mention` | 0.0052 → **0.0059** (+0.0007) |
...
| `typo` | B | CONDITIONAL | 214 | 0.6299 → **0.6299** (0.0) | 0.6308 | 0.0 | 0.6308 | — | 0 |
| `typo_particle` | B | CONDITIONAL | 214 | 0.6558 → **0.6558** (0.0) | 0.6776 | 0.0 | 0.6776 | — | 0 |
...
| `derivative_org` | A | FORBIDDEN | 214 | 1.0 → **1.0** (0.0) | 1.0 | 0.0097 | 1.0 | 108 → **108** (0) | 0 |
| `derivative_role` | A | FORBIDDEN | 214 | 1.0 → **1.0** (0.0) | 1.0 | 0.0097 | 1.0 | 88 → **88** (0) | 0 |
| `derivative_particle` | A | FORBIDDEN | 214 | 1.0 → **1.0** (0.0) | 1.0 | 0.0162 | 1.0 | 94 → **94** (0) | 0 |
| `org_unit` | A | FORBIDDEN | 214 | 1.0 → **1.0** (0.0) | 1.0 | 0.0812 | 1.0 | 135 → **32** (-103) | 0 |
| `artifact` | A | FORBIDDEN | 214 | 1.0 → **1.0** (0.0) | 1.0 | 0.0097 | 1.0 | 214 → **30** (-184) | 0 |
...
- 그중 `UNKNOWN` 판정: 639 → **352** (-287) (카탈로그 확장 여지)

관계 라벨 분포: `UNKNOWN` 352, `ROLE_OF` 186, `ARTIFACT_OF` 184, `PART_OF` 181, `DERIVED_FROM` 112, `NAMED_VARIANT` 55
...
- decoy가 **후보로 올라온** mention: 193 — 0이면 이 시험은 무의미하다(decoy에 닿지도 못했다는 뜻)
- **decoy 확정(FP): 0건**
...
- 확정: decoy 없음 301 → decoy 있음 259 (**차이 42**)
...
*측정 시점: commit `de24ffd`, 2026-09-01. 리포트와 코드가 어긋나면 코드가 맞다 — 재생성해서 확인할 것.*
